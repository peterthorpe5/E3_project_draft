...
Pocket-review pages generated by structural-alignment v0.4.0 include direct
PDF downloads for the current rotated 3D view and complete MAFFT alignment.
Version 0.12.0 also injects the same offline controls into compatible legacy
group pages at load time, so an existing review bundle need not be regenerated.
Every native app graph also has a PDF download; Plotly-native exports require a
working Kaleido renderer, while ggplot-based exports use R's PDF device.
...
Horizontal box plots on the same card show retained selected-pocket scores for
assessed members. A searchable selector lists every structurally assessed group
with member-level scores and defaults to the highest-ranked group reaching the
last gate. The selected-group summary reports its evolutionary-group ID, lead
cluster, assessed-member count, minimum score and complete status at the slider
threshold. An **All groups reaching the last gate** option retains the ranked
comparison view, bounded to 30 groups for readable plotting. Member points are
overlaid on a shared zero-to-one axis and the dashed reference line follows the
selected slider threshold.
