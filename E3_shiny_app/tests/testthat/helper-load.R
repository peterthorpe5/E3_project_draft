...
source(file.path(repo_dir, "R", "fasta_export.R"))
source(file.path(repo_dir, "R", "plot_pdf_export.R"))
...
source(file.path(repo_dir, "R", "glossary.R"))
source(file.path(repo_dir, "R", "ranking_explorer.R"))
source(file.path(repo_dir, "R", "threshold_explorer.R"))
source(file.path(repo_dir, "R", "druggability_visualisations.R"))
source(file.path(repo_dir, "R", "candidate_visualisations.R"))
source(file.path(repo_dir, "R", "structural_alignment_visualisations.R"))
source(file.path(repo_dir, "R", "workflow_schematic.R"))
...
source(file.path(repo_dir, "R", "enriched_hogs.R"))
source(file.path(repo_dir, "R", "prestructure_hog_explorer.R"))
source(file.path(repo_dir, "R", "seed_catalogue.R"))
source(file.path(repo_dir, "R", "unified_search.R"))
source(file.path(repo_dir, "R", "tab_help.R"))
source(file.path(repo_dir, "R", "method_annotations.R"))
...
source(file.path(repo_dir, "R", "module_glossary.R"))
source(file.path(repo_dir, "R", "module_result_section.R"))
source(file.path(repo_dir, "R", "module_threshold_explorer.R"))
source(file.path(repo_dir, "R", "module_candidate_visualisations.R"))
...
source(file.path(repo_dir, "R", "module_prestructure_hog_explorer.R"))
source(file.path(repo_dir, "R", "module_seed_catalogue.R"))
...
make_test_resource_duckdb <- function() {
  testthat::skip_if_not_installed("DBI")
  testthat::skip_if_not_installed("duckdb")
  testthat::skip_if_not_installed("duckplyr")
  ...
  DBI::dbWriteTable(
    ...
    "proteins__proteins",
    tibble::tibble(
      protein_id = c("Q39090", "O80608"),
      species = c("Arabidopsis_thaliana", "Arabidopsis_thaliana"),
      source_file = c("tables/e3_ligases.csv", "tables/e3_ligases.csv")
    )
  )
  DBI::dbWriteTable(
    con,
    ...
    tibble::tibble(
      ...
      parquet_file = "parquet/proteins/proteins.parquet",
      status = "created",
      error = ""
    )
  )
